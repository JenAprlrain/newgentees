...
function isHaiku(text: string) {
  ...
  if (syllables[0] !== 5) {
    toast.error(
      `Line 1 has ${syllables[0]} syllables, it should have 5 syllables`
    );
    return false;
  }
  ...
  if (syllables[1] !== 7) {
    toast.error(
      `Line 2 has ${syllables[1]} syllables, it should have 7 syllables`
    );
    return false;
  }
  ...
  if (syllables[2] !== 5) {
    toast.error(
      `Line 3 has ${syllables[2]} syllables, it should have 5 syllables`
    );
    return false;
  }
...
export function onThaikuExport(
  width: number,
  height: number,
  haikuText: string
) {
  ...
  if (ctx) {
    ...
    img.onload = () => {
      ...
      if (ctx2) {
        ...
        ctx.drawImage(
          canvas2,
          canvas.width / 2 - canvas2.width / 2,
